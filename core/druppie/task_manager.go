...
func (tm *TaskManager) runTaskLoop(task *Task) {
	...
	// --- NATIVE WORKFLOW ENGINE (Loopable Check) ---
	checkNativeWorkflow := func() bool {
		...
		if err == nil {
			for _, s := range p.Steps {
				if s.Status == "pending" || s.Status == "waiting_input" || s.Status == "running" {
		...
		if currentAgentID == "" && len(task.Plan.SelectedAgents) > 0 {
			currentAgentID = task.Plan.SelectedAgents[0]
		}

		if currentAgentID != "" {
			if wf, ok := tm.workflowManager.GetWorkflow(currentAgentID); ok {
				tm.OutputChan <- fmt.Sprintf("[%s] 🚀 Switching to Native Workflow Engine for agent: %s", task.ID, currentAgentID)
				...
				if err != nil {
					task.Status = TaskStatusError
					if p, e := tm.planner.Store.GetPlan(task.ID); e == nil {
						p.Status = "stopped"
						tm.updatePlanCost(&p)
						_ = tm.planner.Store.SavePlan(p)
					}
	...
	for {
		...
		// 3. Native Workflow Check (Loopable)
		if checkNativeWorkflow() {
			return
		}
		...
		for _, idx := range batchIndices {
			...
			isReview := step.Action == "content_review" || step.Action == "draft_scenes" || step.Action == "audit_request" || step.Action == "review_and_governance" || step.Action == "review_governance"
...
`, execErr, step.Params)
			...
			if finishedLast {
				...
				// 1. Capture Usage Before
				// Replanning step creation is now handled by tm.planner.UpdatePlan internally
				...
				if err == nil {
					...
					found := false
					for i := len(updatedPlan.Steps) - 1; i >= 0; i-- {
						// Look for the most recent replanning step Added by UpdatePlan
						if updatedPlan.Steps[i].Action == "replanning" && updatedPlan.Steps[i].Status == "completed" {
							// Update usage and labels
		...
		case "content_review", "draft_scenes":
		...
		case "review_and_governance", "review_governance":
		...
		case answer := <-task.InputChan:
			...
			if activeStep.Action == "content_review" || activeStep.Action == "draft_scenes" || strings.Contains(strings.ToLower(activeStep.Action), "review") {
